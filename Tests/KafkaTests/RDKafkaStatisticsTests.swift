...
struct RDKafkaStatisticsTests {
    ...
    @Test func producerStatisticsPopulateInstruments() throws {
        ...
        withMetricsFactory(metrics) {
            KafkaProducerMetrics(prefix: "kafka").updateFromStatistics(stats)
        }

        // Top-level producer gauges/counters.
        #expect(try metrics.expectGauge("kafka.producer.queue.messages").lastValue == 22710)
        #expect(try metrics.expectGauge("kafka.producer.queue.bytes").lastValue == 704010)
        // First sample emits the full cumulative total as a delta from zero.
        #expect(try metrics.expectCounter("kafka.producer.messages.sent.total").totalValue == 4_300_753)

        // Per-broker latency windows are recorded as avg/p99/max gauges (in milliseconds),
        // dimensioned by broker.
        let broker = [("broker", "localhost:9092/2")]
    ...
    @Test func consumerStatisticsPopulateInstruments() throws {
        ...
        withMetricsFactory(metrics) {
            KafkaConsumerMetrics(prefix: "kafka").updateFromStatistics(stats)
        }

        // queue.operations mirrors `replyq` (0 in the fixture).
        #expect(try metrics.expectGauge("kafka.consumer.queue.operations").lastValue == 0)
        // Rebalances are derived from the authoritative cgrp counter (rebalance_cnt == 2), emitted
        // as a delta from zero on the first sample.
        #expect(try metrics.expectCounter("kafka.consumer.rebalances.total").totalValue == 2)
        // Every partition in the fixture reports consumer_lag == -1 (unknown), which is skipped, so
        // the max-lag gauge settles at 0.
        #expect(try metrics.expectGauge("kafka.consumer.lag.max").lastValue == 0)
